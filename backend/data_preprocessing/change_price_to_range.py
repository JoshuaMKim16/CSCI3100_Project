...
if __name__ == "__main__":
    # 读取 API 密钥（确保文件 google_api_key.env 存在且格式为 "KEY='your_api_key'"）
    with open('google_api_key.env', 'r') as f:
        API_KEY = f.readline().strip().split('=')[1].replace("'", "")
    
    # 读取原始 JSON 文件（假设文件名为 hk_attractions_text_search.json）
    ...
    # 保存更新后的 JSON 文件（新文件名为 hk_attractions_updated.json）
